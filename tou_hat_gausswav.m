...
%%%%%% Computation of denominator coefficients that are independent of beta in touhat transformation  %%%%%%%%%%
for j = 0:k1+p,
    dencoeff(1,j+1) =(-1)^(k1+p-j)* (factorial(k1+p)/(factorial(j)*factorial(k1+p-j)))/((den(1,(k1-j)+1+p)));
end

%%%%%% Computation of numerator coefficients that are independent of beta in touhat transformation  %%%%%%%%%%
for j = 0:k1+p,
...
%%%%%%%%%%%%%%%% beta of the touhat transformation  %%%%%%%%% 
Den = zeros(1,k1+1);
for i=0:k1
